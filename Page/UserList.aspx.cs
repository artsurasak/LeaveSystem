...
namespace IS.Page
{
    public partial class UserList : System.Web.UI.Page
    {
        ...
        private void getUserList()
        {
            ...
            sql += "FROM [LEAVE].[dbo].[USER] usr , [LEAVE].[dbo].[DEPARTMENT] dep , [LEAVE].[dbo].[USER_GROUP] usrGroup ";
            sql += "where usr.DeptID = dep.DEPARTMENT_ID ";
            sql += "and usr.USER_GROUP = usrGroup.GroupID ";
            if (whereClause != "") sql += whereClause;
            sql += ")";
            sql += "select * ";
            sql += ",(select usr.FIRST_NAME + ' ' + usr.LAST_NAME from [LEAVE].[dbo].[USER] usr where DeptID = listUser.DeptID and usr.USER_GROUP = '3') as APPROVE_NAME ";
            sql += ",(select usr.FIRST_NAME + ' ' + usr.LAST_NAME from [LEAVE].[dbo].[USER] usr where DeptID = listUser.DeptID and usr.USER_GROUP = '4') as COMFIRM_NAME ";
            sql += "from listUser ";
