...
namespace IS.Page
{
    public partial class StatusLeave : System.Web.UI.Page
    {
        string Group;
        protected void Page_Load(object sender, EventArgs e)
        {
            Group = Session["Group"].ToString();
            if (Group != "1") tblSearch.Visible = false; 
            bindData();
        }
        ...
        private void bindData() {
            ...
            if (Group == "1") sEmpCode = txtEmpCode.Text;
            ...
            sql += "CASE ";
            sql += "WHEN STATUS = 'A' THEN 'อนุญาติ' ";
            sql += "WHEN STATUS = 'R' THEN 'ไม่อนุญาติ' ";
            ...
            sql += "FROM [LEAVE].[dbo].[REQUEST_LEAVE] reqLeave , [LEAVE].dbo.[USER] usr , [LEAVE].[dbo].[LEAVE_TYPE] lType ";
            ...
            sql += ") ";
            sql += "select * , ";
            sql += "(select FIRST_NAME + ' ' + LAST_NAME from [LEAVE].[dbo].[USER] where EMP_CODE = APPROVE_BY) as ApprName , ";
            sql += "(select FIRST_NAME + ' ' + LAST_NAME from [LEAVE].[dbo].[USER] where EMP_CODE = reqData.empConfirm) as confirmName ";
            sql += "from reqData ";
            ds = db.getData(sql);
