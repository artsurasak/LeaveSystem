...
namespace IS.Page
{
    public partial class HistoryLeave1 : System.Web.UI.Page
    {
        ...
        private void getDataList()
        {
            ...
            sql += "from [LEAVE].[dbo].[REQUEST_LEAVE] reqLeave  , [LEAVE].[dbo].[LEAVE_TYPE] LTYPE ";
            ...
            sql += ")";
            sql += "select * , ";
            sql += "(select usr.FIRST_NAME + ' ' + usr.LAST_NAME from [LEAVE].[dbo].[USER] usr where usr.EMP_CODE = APPROVE_BY and STATUS <> 'รอพิจารณา') as APPROVE_NAME , ";
            sql += "(select usr.FIRST_NAME + ' ' + usr.LAST_NAME from [LEAVE].[dbo].[USER] usr where usr.EMP_CODE = CONFIRM_BY and ConfirmStatus <> 'false') AS COMFIRM_NAME ";
